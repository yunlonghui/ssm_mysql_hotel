...
layui.config({
    ...
    //表格数据
    table.render({
        id: 'reservationDoneList',//设定容器唯一 id。id 是对表格的数据操作方法上是必要的传递条件，它是表格容器的索引，
        elem: '#reservationDoneList',//指定原始 table 容器的选择器或 DOM，方法渲染方式必填
        url:ctx+'/admin/getReservationDoneList', //数据接口 默认会自动传递两个参数：?page=1&limit=30 page 代表当前页码、limit 代//表
        ...
        cols:[[ //设置表头 值是一个二维数组
            ...
            {title:'办理退房',toolbar:'#barEdit', align:'center',width:120}
        ]],
        page:true //开启分页
    });
    ...
    { //test 是 table 原始容器的属性 lay-filter="对应的值
        ...
        if(obj.event==='check_out') //办理退房
        {

            layer.confirm('确定要办理'+data.check_in_name+'退房么？',function (index) {
                $.ajax({
                    url:ctx+"/admin/check_out/"+data.room_info_id+"/"+data.reservation_id,
                    type:"POST",
                    success:function (d) {
                        if(d.code==0)
                        {
                            layer.msg("退房成功",{icon:1});
                            table.reload('reservationDoneList',{})
                        }
                        else
                        {
                            layer.msg("退房失败，请检查错误信息",{icon:5});
                        }
                    },
                    error:function () {
                        layer.msg("退房失败，检查sql及输出",{icon:5});
                    }
                })
                layer.close(index);
            });
