...
layui.config({
    ...
    //表格数据
    table.render({
        id: 'catalogList',//设定容器唯一 id。id 是对表格的数据操作方法上是必要的传递条件，它是表格容器的索引，
        elem: '#catalogList',//指定原始 table 容器的选择器或 DOM，方法渲染方式必填
        url:ctx+'/room/getCatalogList', //数据接口 默认会自动传递两个参数：?page=1&limit=30 page 代表当前页码、limit 代//表
        ...
        cols:[[ //设置表头 值是一个二维数组
            {field:'room_catalog_id',title:'房间类型ID',sort:true,width:80,align:'center'}, //允许排序
            {field:'catalog',title:'类型名称',align:'center'},
    ...
    table.on('tool(test)',function (obj) { //test 是 table 原始容器的属性 lay-filter="对应的值
        ...
        if(obj.event==='del')
        {
            layer.confirm('真的删除吗？',function (index) {
                $.ajax({
                    url:ctx+"/room/delCatalogById/"+data.room_catalog_id, // 发送的是/admin/delAdminById/7
                    type:"get",
                    success:function (d) {
                        if(d.code==0) //删除成功
                        {
                            table.reload('catalogList',{}) //表格重载 这里的表格重载是指对表格重新进行渲染
        ...
        else if(obj.event==='edit')
        {
            layer.open({ //发送请求 controller返回一个jsp页面 加载在这个open的弹出窗口中
                ...
                title:"编辑房间类别信息",
                area:['350px','300px'],
                content:ctx+"/room/editCatalog/"+data.room_catalog_id  // 发送的是/admin/editAdmin/4
            })
        }
    });
    //添加角色 通过选择样式 选择
