...
layui.config({
    ...
    //表格数据
    table.render({
        id: 'reservationList',//设定容器唯一 id。id 是对表格的数据操作方法上是必要的传递条件，它是表格容器的索引，
        elem: '#reservationList',//指定原始 table 容器的选择器或 DOM，方法渲染方式必填
        url:ctx+'/admin/getReservationList', //数据接口 默认会自动传递两个参数：?page=1&limit=30 page 代表当前页码、limit 代//表
        ...
        cols:[[ //设置表头 值是一个二维数组
            ...
            {title:'办理入住',toolbar:'#barEdit', align:'center',width:120}
        ]],
        page:true //开启分页
    });
