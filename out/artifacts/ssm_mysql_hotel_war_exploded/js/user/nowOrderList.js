...
layui.config({
    ...
    //表格数据
    table.render({
        id: 'nowOrderList',//设定容器唯一 id。id 是对表格的数据操作方法上是必要的传递条件，它是表格容器的索引，
        elem: '#nowOrderList',//指定原始 table 容器的选择器或 DOM，方法渲染方式必填
        url:ctx+'/user/getNowOrderList', //数据接口 默认会自动传递两个参数：?page=1&limit=30 page 代表当前页码、limit 代//表
        ...
        cols:[[ //设置表头 值是一个二维数组
            ...
            {title:'取消预约',toolbar:'#barEdit', align:'center',width:120}
        ]],
        page:true //开启分页
    });
    ...
    { //test 是 table 原始容器的属性 lay-filter="对应的值
        ...
        if(obj.event==='cancel') //取消预约
        {

            layer.confirm('确定要取消'+data.roomInfo.room_num+'房间的预约么？',function (index) {
                $.ajax({
                    url:ctx+"/user/cancel/"+data.room_info_id,
                    type:"POST",
                    success:function (d) {
                        if(d.code==0)
                        {
                            layer.msg("取消成功",{icon:1});
                            table.reload('nowOrderList',{})
                        }
                        else
                        {
                            layer.msg("取消失败，请检查错误信息",{icon:5});
                        }
                    },
                    error:function () {
